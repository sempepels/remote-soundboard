...
- **Soundboard remote controller** — runs on the DJ's smartphone (Android/iOS) and on desktop
  (Windows/macOS). Triggers and manages sound effects, hosts the agentic sound processing features, handles
...
- **LangGraph** — the multi-agent AI sound-extraction workflow (parse → locate → extract → validate → preview).
- **LangChain** — LLM wrappers, audio-processing tools, output parsing beneath LangGraph.
...
- **MCP** — exposes agent tools (audio analysis/segmentation, extraction/validation) as MCP servers.
- **LangSmith** — tracing + token/cost accounting (supports the €0.50/extraction cap, PR4).
...
Integrations: **online royalty-free sound libraries** (sound effect retrieval, FR15–FR16/IR10; must be
free per ESR8/DC7), **Spotify Web API** (volume control + ducking, FR23–FR27; needs Spotify Premium on the
...
- **Cost caps.** A single agentic sound extraction request must cost ≤ €0.50 in AI API calls (BR5, PR4); total operational cost ≤ €100/yr (BR13, PR5).
